		...
		<table class="table">
			...
			<c:if test="${not empty list}">
				<c:forEach var="faq" items="${list}">
					...
					<tr>
						<td>${num}</td>
						<c:if test="${faq.fdel!='y'}">
							<td><a href="content.do?fno=${faq.fno}&pageNum=${pageNum}">
									${faq.fquestion}</a></td>
						</c:if>
			...
			<c:if test="${empty list }">
				<tr>
					<th colspan="7" align="center">nothing</th>
				</tr>
			</c:if>
			<tr>
				<th colspan="2">
					<button type="button" class="btn btn-link" onclick="location.href='writeForm.jsp'">Write</button>
					<div align="center">
