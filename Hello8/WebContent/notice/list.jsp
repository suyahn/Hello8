...
<caption>Notice</caption>
	<tr><th>no</th><th width="50%">subject</th>
	<th>date</th><th width="10%">count</th></tr>
...
<!-- items의 list는 listaction의 list를 넘겨준 애 -->
	...
	<c:if test="${notice.ndel!='y'}">
		<td>	
			<a href="content.do?nno=${notice.nno}&pageNum=${pageNum}">
				${notice.nsubject}</a></td>
			<td>${notice.ndate}</td>
			<td>${notice.nreadcount}</td>
	</c:if>	
	<c:set var="num" value="${num - 1}"></c:set></tr>
<!--num은 가상번호 ex 1 2 3 4 5 6 7 8 ~ 거꾸로 하면 9에서 1을 빼야하니깐 8 그리고 1빼서 7 등등  -->
</c:forEach>
</c:if>
<c:if test="${empty list }">
	<tr><th colspan="7">nothing </th></tr>
</c:if>
</table>
</div>
<div align="center">
...
<!--<c:if test="${session.getAttribute('id').equals('master')}">-->
<a href="writeForm.jsp">Write</a>
<a href="delete.do">delete</a><!--</c:if>-->
