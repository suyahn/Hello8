...
<body>
	...
	<table align="center">
		...
		<c:if test="${not empty list2 }">
			<c:forEach var="diary" items="${list2 }">
				<tr>
			...
			<th height="40" colspan="7"><input type="button"
				...
				<div align="center">
					<c:if test="${startPage > PAGEPERBLOCK }">
						<a href="langList.do?pageNum=${endPage - PAGEPERBLOCK}"> <<
							prev </a>
					</c:if>

					<c:forEach var="i" begin="${startPage }" end="${endPage }">

						<a href="langList.do?pageNum=${i }">[${i }] </a>

					</c:forEach>

					<c:if test="${endPage < totalPage }">

						<a href="langList.do?pageNum=${startPage + PAGEPERBLOCK}">
							next >> </a>
